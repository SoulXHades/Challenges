/*
Write an iterative C function levelOrderTraversal prints a level-by-level traversal of the binary tree using a queue,
starting at the root node level.
Note that you should only use enqueue() or dequeue() operations when you add or remove integers from the queue.
...
void levelOrderTraversal(BSTNode* root)
{
    ...
    //by queuing, it will be traversing level-by-level
    //dequeue then enqueue it's children
    while(!isEmpty(head))
    {
