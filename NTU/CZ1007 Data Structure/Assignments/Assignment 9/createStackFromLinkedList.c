...
After the stack is built, write another C function removeEvenValues()
to remove all even integers from the stack. Note that you should only use push() or pop()
when you add or remove integers from stack.



The function prototypes are given as follows:

void createStackFromLinkedList(LinkedList *ll , Stack *stack); void removeEvenValues(Stack *s);
